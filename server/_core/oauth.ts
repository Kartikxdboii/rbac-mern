...
export function registerOAuthRoutes(app: Express) {
  // Optional dev-only fake auth for local testing without external OAuth.
  if (process.env.DEV_FAKE_AUTH === 'true') {
    app.get('/app-auth', async (req: Request, res: Response) => {
      const openId = (typeof req.query.openId === 'string' && req.query.openId) || ENV.ownerOpenId || 'dev-user-001';
      ...
      try {
        ...
        console.warn('[DevAuth] Upsert failed (continuing):', e);
      }
